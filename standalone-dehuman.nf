...
process samtools_filter_and_sort {
    ...
    output:
    tuple ena_sample_name, coguk_id, sample_center, collection_date, received_date, adm0, adm1, run_name, ena_run_name, file("${bam.baseName}.clean.bam"), run_center, l_strategy, l_source, l_selection, run_platform, run_instrument, gisaid_id, min_ct, max_ct, exp_primers, exp_protocol, exp_seq_kit, exp_seq_protocol into extract_manifest_ch

    cpus 4
    memory '5 GB'

    """
    samtools view -h -F4 ${bam} | samtools sort -m1G -@ ${task.cpus} -o ${bam.baseName}.clean.bam
    """
}
...
process extract_bam_reads {
    ...
    input:
    tuple ena_sample_name, coguk_id, sample_center, collection_date, received_date, adm0, adm1, run_name, ena_run_name, file(bam), run_center, l_strategy, l_source, l_selection, run_platform, run_instrument, gisaid_id, min_ct, max_ct, exp_primers, exp_protocol, exp_seq_kit, exp_seq_protocol from extract_manifest_ch
...
process publish_bam {
    ...
    pyena --study-accession ${params.study} --my-data-is-ready --no-ftp \
          ...
          --sample-attr 'geographic location (country and/or sea)' '${adm0}' \
