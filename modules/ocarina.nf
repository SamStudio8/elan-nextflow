process play_ocarina {
    ...
    script:
    """
    ocarina --oauth --angry --profile ${params.ocarina_profile} put file --path "${row.fasta}" --type consensus --i-have-bad-files --source-artifact "sequencing-dummy-reads-${row.run_name}" --bridge-artifact "${row.coguk_id}" --pipeline-hook "bioinfo-${row.run_name}" --sudo-as "${row.username}" --full-path --node climb --publish-group "COG-UK/${row.coguk_id}/${row.seqsite}:${row.run_name}" --meta sequencing platform "${row.platform}";
    ocarina --oauth --angry --profile ${params.ocarina_profile} put file --path "${row.bam}" --type alignment --i-have-bad-files --source-artifact "sequencing-dummy-reads-${row.run_name}" --bridge-artifact "${row.coguk_id}" --pipeline-hook "bioinfo-${row.run_name}" --sudo-as "${row.username}" --full-path --node climb --publish-group "COG-UK/${row.coguk_id}/${row.seqsite}:${row.run_name}";
	...
	do
	   ocarina --oauth --profile ${params.ocarina_profile} put metric --artifact-path ${row.fasta} \
	   ...
	   ocarina --oauth --profile ${params.ocarina_profile} put metric --artifact-path ${row.bam} \
        ...
        ocarina --oauth --angry --profile ${params.ocarina_profile} put qc --publish-group "COG-UK/${row.coguk_id}/${row.seqsite}:${row.run_name}" --test-name 'cog-uk-elan-minimal-qc' --test-version 1;
        ocarina --oauth --angry --profile ${params.ocarina_profile} put qc --publish-group "COG-UK/${row.coguk_id}/${row.seqsite}:${row.run_name}" --test-name 'cog-uk-high-quality-public' --test-version 1;
    """
}
